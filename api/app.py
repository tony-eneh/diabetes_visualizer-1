...
def upload_file():
    ...
    if file:
        ...
        avg_blood_sugar, peak_blood_sugar, trends, insights, recommendations = process_data(data)
...
def process_data(data):
    ...
    data['SMA'] = data['BloodSugar'].rolling(window=5).mean()  # 5-day moving average
    data['Trend'] = np.where(data['SMA'] > data['BloodSugar'], 'Decreasing', 'Increasing')
    
    trends = "Increasing" if (data['SMA'].iloc[-1] > data['SMA'].iloc[0]) else "Decreasing"
